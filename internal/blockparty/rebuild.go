...
func (p *plan) finish(ctx context.Context) error {
	...
	mapOffset := func(old uint32) (uint32, error) {
		...
		if entry.removed || len(encoded) != len(entry.raw) {
	...
	for _, entry := range p.order {
		...
		encoded := p.encodedInstruction(entry)
		copy(output[start:], encoded)
		if connector := p.connectors[entry]; connector != nil {
			position := start + uint32(len(encoded))
			if connector.near {
				copy(output[position:], []byte{0xe9, 0, 0, 0, 0})
			} else {
				copy(output[position:], []byte{0xeb, 0})
			}
		}
	}
...
func (p *plan) relaxBranches(ctx context.Context) error {
	for _, entry := range p.instructions {
		if entry.region != nil && entry.region.isFunction && entry.hasFlow && !entry.flowReloc && (entry.reference.kind == relativeJump || entry.reference.kind == relativeConditional) && !entry.removed {
			entry.variant = branchShort
		}
	}
	limit := len(p.instructions) + len(p.connectors) + 1
	for iteration := 0; iteration <= limit; iteration++ {
		...
		changed := false
		for _, entry := range p.order {
			if entry.region == nil || !entry.region.isFunction || entry.removed || !entry.hasFlow || entry.flowReloc {
				continue
			}
		...
		for entry, connector := range p.connectors {
			if connector.near {
				continue
			}
			target, ok := starts[connector.target]
			if !ok {
				return malformed("branch relaxation", fmt.Errorf("connector target %#x was not laid out", connector.target))
			}
			start := starts[entry.oldStart] + uint32(len(p.encodedInstruction(entry)))
			displacement := int64(target) - int64(start+2)
			if displacement < math.MinInt8 || displacement > math.MaxInt8 {
				connector.near = true
				changed = true
...
func (p *plan) patchControlFlow(output []byte, starts map[uint32]uint32, mapOffset func(uint32) (uint32, error)) error {
	for _, entry := range p.order {
		...
		encoded := p.encodedInstruction(entry)
		start := starts[entry.oldStart]
		if entry.region != nil && entry.region.isFunction && entry.hasFlow && !entry.flowReloc {
			target64, err := relativeTarget(entry)
			if err != nil || target64 < 0 || target64 >= int64(len(p.text.Data)) {
			...
			if err := writeDisplacement(output, start+uint32(fieldOffset), fieldSize, displacement); err != nil {
				return fmt.Errorf("blockparty: branch %#x: %w", entry.oldStart, err)
			}
		}
		if entry.region != nil && entry.region.isFunction && p.object.Machine == coff.MachineAMD64 && hasRIPOperand(entry.operands) && !p.hasRelocationCoveringRIP(entry) {
			...
			if oldTarget < 0 || oldTarget >= int64(len(p.text.Data)) || p.labels[uint32(oldTarget)] == nil {
				return unsupported(entry, fmt.Sprintf("unrelocated RIP-relative target %#x has no local code symbol", oldTarget))
			}
		...
		if connector := p.connectors[entry]; connector != nil {
			target, ok := starts[connector.target]
			if !ok {
				return malformed("connector repair", fmt.Errorf("target %#x was not laid out", connector.target))
			}
			position := start + uint32(len(encoded))
			length := 2
			field := position + 1
			...
			displacement := int64(target) - int64(position+uint32(length))
			fieldSize := 1
			if connector.near {
				fieldSize = 4
			}
			if err := writeDisplacement(output, field, fieldSize, displacement); err != nil {
				return fmt.Errorf("blockparty: connector after %#x: %w", entry.oldStart, err)
			}
		}
	}
	return nil
...
func hasRIPOperand(operands string) bool {
...
func (p *plan) hasRelocationCoveringRIP(entry *instruction) bool {
	for _, relocation := range entry.relocations {
		...
		// With no cs_detail displacement offset, accept only the provable
		// legacy ModRM RIP+disp32 shape whose displacement ends the encoding.
		// Immediate-bearing and VEX/EVEX forms fail closed below.
...
// ripDisplacementField recognizes the three x64 forms LocalLabels processes:
// LEA r64,m; MOV r64,r/m64; and CALL r/m64. VEX/EVEX, address-override,
// unusual prefixes, and every other RIP-relative form fail closed.
...
func (p *plan) rebasedRelocationAddresses(starts map[uint32]uint32) (map[*coff.Relocation]uint32, error) {
	...
	for _, entry := range p.instructions {
		...
		for _, relative := range entry.relocations {
			if uint64(relative.offset)+uint64(relative.width) > uint64(len(encoded)) {
				return nil, malformed("relocation rebasing", fmt.Errorf("relocation offset escaped instruction %#x", entry.oldStart))
			}
...
func (p *plan) codeLayoutChanged(starts map[uint32]uint32, total int) bool {
	...
	for _, entry := range p.instructions {
		if starts[entry.oldStart] != entry.oldStart || len(p.encodedInstruction(entry)) != len(entry.raw) || p.connectors[entry] != nil {
			return true
		}
	}
	return false
...
func (p *plan) validateExistingUnwind(layoutChanged bool) error {
	if !layoutChanged || p.object.Machine != coff.MachineAMD64 {
		return nil
	}
	for _, section := range p.object.Sections {
