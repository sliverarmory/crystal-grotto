...
type boundedRandom interface {
	nextInt(bound int) (value int, draws int, err error)
}
...
func newBoundedRandom(options Options) (boundedRandom, error) {
	if options.Random != nil && options.Seed != nil {
		return nil, errors.New("shatter: Random and Seed are mutually exclusive")
	}
	if options.Seed != nil {
...
func shuffleBlocks(blocks []*block, random boundedRandom) (int, error) {
	draws := 0
