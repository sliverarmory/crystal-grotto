...
func TestEmitPICOptionalDataAndBSS(t *testing.T) {
	...
	if got := string(plain.Bytes); got != "T" {
		t.Fatalf("default x86 PIC = %q, want T", got)
	}
	extended, err := EmitPIC(object, PICOptions{IncludeData: true, IncludeBSS: true})
	if err != nil {
		t.Fatal(err)
	}
	if got, want := len(extended.Bytes), 8; got != want {
		t.Fatalf("extended PIC length = %d, want %d", got, want)
	}
	if placement, ok := extended.Layout.Placement(data); !ok || placement.Offset != 1 {
		t.Fatalf("data placement = %#v, %v", placement, ok)
	}
	if placement, ok := extended.Layout.Placement(bss); !ok || placement.Offset != 4 {
