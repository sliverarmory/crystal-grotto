...
type boundedRandom interface {
	nextInt(bound int) (int, int, error)
}
...
func (r readerRandom) nextInt(bound int) (int, int, error) {
	...
	for {
		...
		// Match java.util.Random.nextInt(bound)'s signed overflow test.
...
func newBoundedRandom(options Options) (boundedRandom, error) {
	if options.Random != nil && options.Seed != nil {
		return nil, errors.New("regdance: Random and Seed are mutually exclusive")
	}
	if options.Seed != nil {
...
func shuffle(registers []Register, random boundedRandom) (int, error) {
	draws := 0
	for size := len(registers); size > 1; size-- {
		index, consumed, err := random.nextInt(size)
		draws += consumed
		if err != nil {
			return draws, err
		}
		registers[size-1], registers[index] = registers[index], registers[size-1]
	}
	return draws, nil
}
